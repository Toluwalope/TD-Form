...
class Wizard extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			nextButton:
					...
					this.props.steps.length > 1 ? true :
					false,
	...
	async componentDidMount() {
		...
		if (window.location.pathname.split('/').length === 3) {
			let id = window.location.pathname.split('/')[2];
			try {
				const populatedData = await axios.get(`http://localhost:4000/api/form/get/${id}`);
			...
			if (validationState) {
				this.setState({
					...
					nextButton:
							...
							this.props.steps.length > key + 1 ? true :
							false,
					previousButton:
	...
	callbackHit = async () => {
			...
			if (allStates.hasOwnProperty('_id')) {
				try {
					if (allStates.competition.baskets.length > 0) {
						const response = await axios.post(`http://localhost:4000/api/form/step2/add`, {
						...
						await axios.put('http://localhost:4000/api/form/update/' + allStates._id, {
							competition: allStates.competition.baskets
						});
					}
		...
		if (allStates.hasOwnProperty('_id')) {
			try {
				...
				const populatedData = await axios.put('http://localhost:4000/api/form/update/' + allStates._id, {
					...otherData,
					currentStep
				});
				let { response } = populatedData.data;
			...
			try {
				...
				const populatedData = await axios.post('http://localhost:4000/api/form/step1/add', {
					...otherData,
					currentStep
				});
				let { response } = populatedData.data;
					...
					<Card className={classes.card}>
						...
						>
							<Grid item sm={12}>
								<LinearProgress
									variant="determinate"
									value={this.state.currentStep * 12.5}
									valueBuffer={12.5}
							...
							<Grid item style={{ margin: 'auto' }}>
								<h5 style={{ fontSize: '1.5rem' }}>{`${Math.floor(
									this.state.currentStep / 8 * 100
								)}%`}</h5>
							</Grid>
						</Grid>
						...
						<div className={classes.footer}>
							...
							<div className={classes.right}>
								{!(this.state.currentStep === 8) && (
									<Button onClick={this.saveNowButton} style={{ marginRight: '1rem' }} color="rose">
										Save Now
									</Button>
								)}
								{
									...
									null}
								{
									this.state.finishButton ? <Button
										color="rose"
